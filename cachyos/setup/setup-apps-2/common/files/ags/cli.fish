...
function build
    ...
    mkdir -p $build_dir
    bun build --minify --target bun --external gi://\* --external file://\* --external resource://\* --external system --external console --external cairo --external gettext --outdir=$build_dir -- $parent_dir/src/main.ts
end
