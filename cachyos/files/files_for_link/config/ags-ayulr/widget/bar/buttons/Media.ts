...
const { length, direction, preferred, monochrome, format, charactersToStrip } =
   options.bar.media;
   ...
   const revealer = Widget.Revealer({
      ...
      child: Widget.Label({
         ...
         label: Utils.merge(
            ...
            () => {
               let track_title = stripCharacters(
                  player.track_title,
                  charactersToStrip.value
               );
               let track_artists = stripCharacters(
                  player.track_artists.join(", "),
                  charactersToStrip.value
               );
               let track_artist = stripCharacters(
                  player.track_artists[0] || "",
                  charactersToStrip.value
               );
               let track_album = stripCharacters(
                  player.track_album,
                  charactersToStrip.value
               );
               let name = stripCharacters(player.name, charactersToStrip.value);
               let identity = stripCharacters(
                  player.identity,
                  charactersToStrip.value
               );
